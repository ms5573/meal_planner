...
const generateMealPlan = (targetMacros, perMealMacros, mealsPerDay, selectedFoodSources, dietaryRestrictions = [], avoidFoods = [], cuisinePreferences = [], numberOfVariations = 20) => {
  // Ensure avoidFoods is always an array
  const avoidFoodsArray = Array.isArray(avoidFoods) ? avoidFoods : [];
  ...
  for (let i = 0; i < numberOfVariations; i++) {
    ...
    const filteredFoodDatabase = foodDatabase.filter(food => {
      ...
      if (avoidFoodsArray.some(item => food.ingredients.includes(item))) return false;
    ...
    mealPlanVariations.push({ meals, actualMacros: dailyMacros });
  }

  return mealPlanVariations;
};
